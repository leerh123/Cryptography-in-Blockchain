...
#该函数用于生成d2，同时结合P1生成公钥P
    ...
    def genrateP_d(self):
        ...
        d=(self.d2*self.d1)%n
        d = (self.inverse_mod_prime(d, n)-1)%n
        self.public_key =self._kg(d,self.ecc_table['g'])
        self.private_key=str(hex(d))


#计算T1
    def compute_T1(self,data):
        n = int(self.ecc_table['n'], base=16)
        len_2 = 2 * self.para_len
        len_3 = len_2 + 64
        data=data.hex()
        self.data=data
        self.C1 = data[0:len_2]
        self.C3 = data[len_2:len_3]
        self.C2 = data[len_3:]
        if self.C1==0:
            return None
        tmp=self.inverse_mod_prime(self.d1, n)
        T1=self._kg(tmp,self.C1)
        return T1

#计算T2
    def compute_T2(self,T1):
        n = int(self.ecc_table['n'], base=16)
        T2=self.inverse_mod_prime(self.d2, n)
        T2=self._kg(T2,T1)
        return T2

#进行解密
    def decrypt_2P(self,T2):
        len_2 = 2 * self.para_len
        x1 = int(self.C1[0:self.para_len], 16)
        y1 = int(self.ecc_table['p'], 16)-int(self.C1[self.para_len:len_2], 16)
        C1_re=(str(hex(x1))+str(hex(y1))[2:])[2:]
        xy = '%s%s' % (T2, 1)
        xy=self._add_point(xy, C1_re)
        xy = self._convert_jacb_to_nor(xy)
        x2 = xy[0:self.para_len]
        y2 = xy[self.para_len:len_2]

        ml = len(self.C2)
        t = sm3.sm3_kdf(xy.encode('utf8'), ml/2)
        M__=int(self.C2, 16)^int(t, 16)
        M__=str(hex(M__)[2:])
        u = sm3.sm3_hash([
                i for i in bytes.fromhex('%s%s%s'% (x2,M__,y2))
            ])
        if u==self.C3:
            return bytes.fromhex(M__)
...
print('\n-------------A生成私钥d1,并生成P1发送给B----------------')
sm2_2P_dec_A = CryptSM2(public_key=None, private_key=None)
P1=sm2_2P_dec_A.genrateP1()
print('d1')
print(sm2_2P_dec_A.d1)
print('P1')
print(P1)

print('\n-------------B生成私钥d2,并生成P公布----------------')
sm2_2P_dec_B = CryptSM2(public_key=None, private_key=None)
P=sm2_2P_dec_B.genrateP(P1)
print('d2')
print(sm2_2P_dec_B.d2)
print('P')
print(P)

print('\n-------------C通过公布的P生成密文----------------------')
sm2_2P_enc_C = CryptSM2(public_key=P, private_key=None)
msg=b'11111111111'
enc_data = sm2_2P_enc_C.encrypt(msg)
print('密文')
print(enc_data)

print('\n--------------A生成T1，发送给B-----------------------')
T1=sm2_2P_dec_A.compute_T1(enc_data)
print('T1')
print(T1)

print('\n--------------B生成T2，发送给A-----------------------')
T2=sm2_2P_dec_B.compute_T2(T1)
print('T2:')
print(T2)

print('\n--------------A进行解密---------------------')
M=sm2_2P_dec_A.decrypt_2P(T2)
print(M)



'''
sm2_2P_enc_C.d1=sm2_2P_dec_A.d1
sm2_2P_enc_C.d2=sm2_2P_dec_B.d2
sm2_2P_enc_C.genrateP_d()
M_yan=sm2_2P_enc_C.decrypt(enc_data)
print(M_yan)
