...
运行指导:
我们假设双方已提过CA获得了彼此的公钥
首先我们需要设定会话密钥key（在最开头的位置）和data即需发送的消息(在主函数部分)
然后通过SM2_PGP_enc(data)函数,获取PGP加密后的消息message来模拟发送方A的PGP加密过程
再然后通过SM2_PGP_dec(message)来模拟，接受方B得到PGP密文后的解密过程
